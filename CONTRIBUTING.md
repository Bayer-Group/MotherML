...
A bug fix or enhancement is delivered via a pull request. Keep each pull request focused on a single change so that reviews stay quick and predictable.

1. Fork the bayer-group/mother-ml repository into your account.
2. Clone your fork and switch into the project directory:

   ```sh
   git clone git@github.com:your_name_here/bayer-group/mother-ml.git
   cd mother-ml
   ```

3. Review the available automation tasks:

   ```sh
   uv run poe help
   ```

4. Install the development dependencies and activate the virtual environment:

   ```sh
   uv sync --group dev --group docs --group test_duration
   source .venv/bin/activate
   ```

   The `.venv` folder is created under the project root and is already ignored by git. An active environment typically prefixes your prompt with `(mother-ml)`.

## Develop a Fix or Enhancement

- Implement the code changes (for example, update modules under `mother-ml/`, supporting scripts, or documentation).
- Add or update unit tests to verify the new behavior.
...
- Check coverage for the affected areas:
  ...
  Coverage reports are written to `coverage-html/index.html`; open the HTML report to inspect gaps.

- Ensure the change meets style conventions:

  ```sh
  uv run poe check-style
  ```

  Use `uv run poe style` to apply formatting and linter fixes automatically when needed.

- Run static analysis checks:

  ```sh
  uv run poe check-static-analysis
  ```

- Update documentation and the change log for non-trivial changes:

  ```sh
  uv run poe docs
  ```

  For a quicker consistency pass, run `uv run poe check-docs`.

- Fix any regressions flagged by tests or analysis.

## Submit Your Changes

- Stage, commit, and push the updates to your fork:
...
- Open a pull request and ensure continuous integration checks pass. Address any feedback or failures promptly.
