...
def _regression_pool(n=90, n_pool=20, n_features=6, seed=0):
    ...
    X, y = make_regression(
        n_samples=n + n_pool, n_features=n_features, noise=0.3, random_state=seed
    )
...
def test_acquisition_flow_head(method):
    ...
    reg = FlowHeadRegressor(
        flow_type="NICE", max_epochs=8, lr=1e-2, device="cpu", verbose=0
    )
    reg.fit(X_tr, y_tr)
...
def test_balsa_kl_grid_fixed_and_adaptive_agree_in_shape():
    ...
    fixed = acquisition_score(
        reg, X_pool, method="balsa_kl_grid", grid_range=(-4.0, 4.0)
    )
...
def test_invalid_method_raises():
    X_tr, y_tr, X_pool = _regression_pool()
    reg = FlowHeadRegressor(
        flow_type="NICE", max_epochs=5, lr=1e-2, device="cpu", verbose=0
    )
    reg.fit(X_tr, y_tr)
