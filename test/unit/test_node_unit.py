...
class RecordingConstantTreeLayer(nn.Module):
    """ODST stand-in that records its input and returns fixed tree outputs."""

    def __init__(self, input_dim, num_trees, tree_output_dim, **kwargs):
        super().__init__()
        del input_dim, kwargs
        self.inputs = []
        self.register_buffer("tree_outputs", torch.arange(1, num_trees * tree_output_dim + 1, dtype=torch.float32))

    def forward(self, inputs):
        self.inputs.append(inputs.detach().clone())
        return self.tree_outputs.expand(inputs.shape[0], -1)


def test_dense_odst_dropout_scopes_separate_input_and_tree_regularization():
    """Input-only dropout leaves inter-layer tree channels to tree dropout."""
    features = torch.tensor([[10.0, 20.0]])
    raw_tree_outputs = torch.arange(1, 7, dtype=torch.float32).reshape(1, 3, 2)
    ...
    first_layer_output = output[:, :3, :].flatten(start_dim=1)
    second_layer_input = block[1].inputs[0]
    assert torch.equal(second_layer_input[:, :2], torch.zeros_like(features))
    assert torch.equal(second_layer_input[:, 2:], first_layer_output)

    # Each tree's two output dimensions are either both dropped or both rescaled.
    tree_output_ratio = first_layer_output.reshape(1, 3, 2) / raw_tree_outputs
    assert torch.all((tree_output_ratio == 0) | (tree_output_ratio == 2))
    assert torch.equal(tree_output_ratio[..., 0], tree_output_ratio[..., 1])

    legacy_block = DenseODSTBlock(
        input_dim=2,
        num_trees=3,
        num_layers=2,
        tree_output_dim=2,
    ...
    legacy_block.train()
    legacy_block(features)
    assert torch.equal(legacy_block[1].inputs[0], torch.zeros(1, 8))


def test_dense_odst_tree_dropout_is_applied_once_per_layer_or_deferred_to_head():
    """Tree dropout is layer-local when configured, otherwise deferred to the head."""
    features = torch.ones(2, 2)

    layer_dropout_block = DenseODSTBlock(
        input_dim=2,
        num_trees=3,
        num_layers=2,
        tree_output_dim=2,
    ...
    layer_dropout_calls = 0
    original_apply_tree_dropout = layer_dropout_block._apply_tree_dropout

    def count_layer_dropout_calls(layer_output):
        nonlocal layer_dropout_calls
        layer_dropout_calls += 1
        return original_apply_tree_dropout(layer_output)

    layer_dropout_block._apply_tree_dropout = count_layer_dropout_calls
    layer_dropout_block.train()
    layer_dropout_block(features)
    assert layer_dropout_calls == 2

    deferred_dropout_block = DenseODSTBlock(
        input_dim=2,
        num_trees=3,
        num_layers=2,
        tree_output_dim=2,
    ...
    deferred_dropout_calls = 0
    original_deferred_apply_tree_dropout = deferred_dropout_block._apply_tree_dropout

    def count_deferred_dropout_calls(layer_output):
        nonlocal deferred_dropout_calls
        deferred_dropout_calls += 1
        return original_deferred_apply_tree_dropout(layer_output)

    deferred_dropout_block._apply_tree_dropout = count_deferred_dropout_calls
    deferred_dropout_block.train()
    deferred_dropout_block(features)
...
class DropoutScopeTrial:
    ...
    def __init__(self, input_only):
        self.input_scope = input_only
        self.float_bounds = {}

    def suggest_int(self, name, low, high, **kwargs):
        del name, high, kwargs
        return low

    def suggest_float(self, name, low, high, **kwargs):
        del high, kwargs
        self.float_bounds[name] = low
        return low

    def suggest_categorical(self, name, choices):
        if name == "input_dropout_only_input":
            return self.input_scope
        if name == "tree_dropout_only_head":
            return True
        return choices[0]
...
@pytest.mark.parametrize(
    ("input_only", "expected_tree_only_head"),
    [
        (False, True),
        (True, True),
    ],
)
def test_node_dropout_tuning_allows_deterministic_models(input_only, expected_tree_only_head):
    """Both dropout rates can tune to zero regardless of the chosen scope."""
    trial = DropoutScopeTrial(input_only)
    model = NODERegressor(num_layers=2, tune_head=False, head_type="subset", device="cpu")
    params = model.get_hyperparameter_space(np.zeros((2, 2)), np.zeros(2), trial)

    assert trial.float_bounds["input_dropout"] == 0.0
    assert trial.float_bounds["tree_dropout"] == 0.0
    assert params["tree_dropout_only_head"] == expected_tree_only_head


def test_one_layer_dropout_tuning_uses_non_redundant_defaults():
    trial = DropoutScopeTrial(True)
    model = NODERegressor(num_layers=1, tune_head=False, head_type="subset", device="cpu")
    params = model.get_hyperparameter_space(np.zeros((2, 2)), np.zeros(2), trial)

    assert params["input_dropout_only_input"] is True
    assert params["tree_dropout_only_head"] is True
...
def test_max_layers_retained_tuning_is_adaptive():
    ...
    assert params_single["num_trees"] == 256
    ...
    assert params_two_layers["num_trees"] == 128
    ...
    assert params_four_layers["num_trees"] == 128
