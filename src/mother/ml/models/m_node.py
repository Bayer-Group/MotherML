...
from mother.ml.models.node_utils import (
    DenseODSTBlock,
    Embedding1dLayer,
    Lambda,
    balsa_emd_from_mc_samples,
    entmax15,
    entmoid15,
    sparsemax,
    sparsemoid,
...
class NODEModel(nn.Module):
    ...
    def _build_head(self) -> nn.Module:
        ...
        if head_type == "subset":
            ...
            flow_type = getattr(self.hparams, "flow_type", "NSF")
...
class CompletePyTorchTabularNODE(nn.Module):
    ...
    Args:
        ...
        input_dropout: Feature-wise dropout on the input of every ODST layer.
        input_dropout_only_input: If True, apply input dropout only to original
            input features; if False, include between-layer tree outputs.
        ...
        tree_dropout: Probability of dropping entire trees.
        tree_dropout_only_head: If True, apply tree dropout only before the head;
            if False, apply it to every ODST layer output.
    ...
    def __init__(
        ...
        initialize_response: str = "normal",  # "uniform" or "normal"
        ...
        tree_dropout: float = 0.0,  # drop entire trees (regularization)
        tree_dropout_only_head: bool = True,
        flow_type: str = "NSF",  # Flow architecture; only for head_type="flow"
    ...
    def _build_modules(self) -> None:
        ...
        self.dense_block = DenseODSTBlock(
            ...
            tree_dropout_only_head=self.tree_dropout_only_head,
            depth=self.depth,
            choice_function=choice_func,
            bin_function=bin_func,
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # trees as each layer produced them, so we must not draw a second mask here.
        if self.training and getattr(self, "tree_dropout", 0) > 0 and getattr(self, "tree_dropout_only_head", True):
            # x is [batch, num_layers * num_trees, tree_output_dim]; the mask has a
            # trailing singleton dim so one draw per tree covers all its outputs.
...
class BaseNODEEstimator(NeuralNet, AbstractMotherPipeline):
    ...
    def _store_node_parameters(
        ...
        flow_signal: int,
        flow_components: int,
        batch_size_tuning_upper_bound: Optional[int],
        ...
        self.flow_signal = flow_signal
        self.flow_components = flow_components
        self.batch_size_tuning_upper_bound = batch_size_tuning_upper_bound
        self.cat_features = cat_features
        self.callbacks = callbacks
        self._original_callbacks = callbacks

    def _batch_size_candidates(self) -> List[int]:
        """Build batch-size candidates for tuning from an optional upper bound.

        If ``batch_size_tuning_upper_bound`` is unset, tuning keeps a single
        fixed candidate equal to the estimator's current ``batch_size``.
        When an upper bound is provided, tuning starts from the user-provided
        ``batch_size`` and repeatedly doubles it until the bound is reached.
        """
        upper_bound = getattr(self, "batch_size_tuning_upper_bound", None)
        start_batch_size = int(self.batch_size)

        if upper_bound is None:
            return [start_batch_size]

        if upper_bound < 64:
            raise ValueError("batch_size_tuning_upper_bound must be >= 64 when provided")
        if start_batch_size > int(upper_bound):
            raise ValueError("batch_size must be <= batch_size_tuning_upper_bound when provided")

        candidates = [start_batch_size]
        next_candidate = start_batch_size * 2
        while next_candidate <= int(upper_bound):
            candidates.append(next_candidate)
            next_candidate *= 2
        unique_sorted = sorted(set(candidates))
        return unique_sorted if unique_sorted else [start_batch_size]
    ...
    @contextmanager
    def _temporary_dropout_rates(
        self,
        *,
        input_dropout: Optional[float] = None,
        tree_dropout: Optional[float] = None,
    ) -> Iterator[None]:
        """Temporarily override custom NODE dropout rates during inference.

        ``None`` preserves the fitted rate. Overrides are restored even when a
        prediction call fails and deliberately do not affect MLP-head dropout.
        """
        overrides = {"input_dropout": input_dropout, "tree_dropout": tree_dropout}
    ...
    def _warn_zero_dropout_mc(self, context: str) -> None:
        ...
        msg = (
            f"{context}: input_dropout, tree_dropout, and relevant head dropout are all 0. "
            "MC-dropout repeats are deterministic, so variance-based epistemic uncertainty "
            "collapses to zero. Set at least one dropout > 0 during training, or pass "
            "input_dropout=<rate> and/or tree_dropout=<rate> to predict_uncertainty() "
            "for a temporary inference-time override."
        )
    ...
    def _build_skorch_init_params(
        ...
        **extra: Any,
        ...
        return dict(
            ...
            module__tree_dropout_only_head=self.tree_dropout_only_head,
    ...
    def _predict_uncertainty_mc_dropout(
        ...
        has_dropout = self._has_active_dropout(include_mlp=True)

        # If no dropout configured, fall back to regular predict
        if not has_dropout:
            self._warn_zero_dropout_mc("predict_uncertainty (MC-dropout)")
            if is_classifier:
    ...
    def get_hyperparameter_space(
        ...
        ``num_trees`` is *derived* rather than sampled (``total_trees // num_layers``,
        keeping total capacity constant across depths) and recorded as a trial user
        attribute for traceability.
        """
        num_layers = trial.suggest_int(prefix + "num_layers", 1, 8, log=False)

        # The tuned quantity is the TOTAL tree budget; the per-layer ``num_trees``
        # handed to the model is derived from it, so capacity stays comparable
        # across depths and Optuna never models a value the estimator ignores.
        total_trees = trial.suggest_int(prefix + "total_trees", 256, 2048, step=256, log=False)
        num_trees = max(1, total_trees // num_layers)
        trial.set_user_attr(prefix + "num_trees", num_trees)

        suggested_params = {
            prefix + "num_layers": num_layers,
            prefix + "num_trees": num_trees,
            ...
            prefix + "batch_size": trial.suggest_categorical(prefix + "batch_size", self._batch_size_candidates()),
        }
        ...
        # Tune dropout parameters freely as architectural regularization. A trial
        # with every applicable rate set to zero is a valid deterministic model;
        # predict_uncertainty() warns when its MC estimate is then meaningless.
        if num_layers > 1:
            input_dropout_only_input = trial.suggest_categorical(prefix + "input_dropout_only_input", (False, True))
        else:
            input_dropout_only_input = True
        suggested_params[prefix + "input_dropout_only_input"] = input_dropout_only_input
        suggested_params[prefix + "input_dropout"] = trial.suggest_float(
            prefix + "input_dropout", 0.0, 0.15, step=0.001
        )
        suggested_params[prefix + "tree_dropout"] = trial.suggest_float(prefix + "tree_dropout", 0.0, 0.35, step=0.001)
        if num_layers > 1:
            suggested_params[prefix + "tree_dropout_only_head"] = trial.suggest_categorical(
                prefix + "tree_dropout_only_head", (False, True)
            )
        else:
            suggested_params[prefix + "tree_dropout_only_head"] = True
    ...
    def default_parameters(self, prefix: str = "") -> Dict[str, Any]:
        ...
        Keys must match the names Optuna *samples* (e.g. ``total_trees``, not the
        derived ``num_trees``) because this dict is enqueued as the first trial.

        These defaults are intentionally aligned with Mother's preferred
        one-layer baseline profile so the constructor defaults and the
        MotherTuner startup/enqueued parameters begin from the same practical
        configuration.
        """
        return {
            prefix + "lr": 0.005,
            prefix + "batch_size": int(self.batch_size),
            prefix + "depth": 4,
            prefix + "num_layers": 1,
            prefix + "total_trees": 512,
            prefix + "additional_tree_output_dim": 3,
            prefix + "choice_function": "entmax15",
            prefix + "bin_function": "entmoid15",
            prefix + "input_dropout": 0.05,
            prefix + "input_dropout_only_input": True,
            prefix + "tree_dropout": 0.02,
            prefix + "tree_dropout_only_head": True,
...
class NODERegressor(BaseNODEEstimator):
    ...
    Key Features:
        ...
        - Mixed data types: continuous and categorical features. Categorical
          ...
          CatBoost); they are never auto-detected.
                - Optional batch-size tuning with ``batch_size_tuning_upper_bound``;
                    tuning starts from the user-set ``batch_size`` and explores larger
                    doubled sizes up to the bound.
        - DataFrame and numpy array support
        - Validation split + early stopping are opt-in via ``train_split``
    ...
    Example:
        ...
        >>> # Enable validation + early stopping (opt-in):
        >>> from skorch.dataset import ValidSplit
        >>> reg_es = NODERegressor(
        ...     train_split=ValidSplit(cv=0.15, random_state=42),
        ... )
        >>> reg_es.fit(X_train, y_train)

        >>> # Override early stopping settings via callbacks:
        >>> from skorch.callbacks import EarlyStopping
        >>> reg_es_custom = NODERegressor(
        ...     train_split=ValidSplit(cv=0.15, random_state=42),
        ...
        >>> # Batch-size tuning with only an upper bound:
        >>> # starts at batch_size=128 and can try 256, 512, 1024
        >>> reg_tune_bs = NODERegressor(batch_size=128, batch_size_tuning_upper_bound=1024)
    ...
    def __init__(
        ...
        # ====================================================================
        head_type: str = "subset",  # "subset", "linear", "mlp", or "flow" (probabilistic)
        ...
        flow_type: str = "NSF",  # Flow architecture (if head_type="flow")
        flow_transforms: int = 3,  # Transform layers (NICE, RealNVP, NAF, UNAF)
        flow_bins: int = 8,  # Spline bins (NSF)
        flow_degree: int = 16,  # Polynomial degree (BPF)
        flow_signal: int = 16,  # Hidden signal dim (NAF, UNAF)
        ...
        input_dropout_only_input: bool = False,
        tree_dropout: float = 0.02,  # Mild tree dropout regularization
        tree_dropout_only_head: bool = True,
        ...
        batch_size_tuning_upper_bound: Optional[int] = 512,  # Upper bound for tuning batch_size
    ...
    def predict(
        ...
        Args:
            ...
            return_sample_distribution: If ``True`` and ``head_type='flow'``,
                ...
                ``(n_molecules, num_samples, output_dim)`` so that
                ``result[i]`` gives all samples for molecule ``i``. For
                non-flow heads this option is not supported and raises
                ``ValueError``.

        Returns:
            Predictions array, or full sampled flow distribution with shape
            ``(n_molecules, num_samples, output_dim)`` when
    ...
    def predict_flow_head(
        ...
        Args:
            ...
            return_sample_distribution: If ``True``, return raw samples from the
                learned flow with shape ``(n_molecules, num_samples, output_dim)``
                instead of mode predictions, so that ``result[i]`` gives all
                samples for molecule ``i``.

        Returns:
            Predictions (mode) from the flow distribution, or raw sampled
            distribution with shape ``(n_molecules, num_samples, output_dim)``
            if ``return_sample_distribution=True``.
    ...
    def predict_uncertainty(
        ...
        **kwargs: Any,
        ...
        By default, both override parameters are ``None`` and the model uses the
        dropout rates learned/configured during training. Supplying one or both
        rates is useful for sensitivity analysis or when a downstream workflow
        explicitly requires stochastic MC passes. Overrides apply only for this
        inference call and are restored afterwards; they never modify the fitted
        estimator or its subsequent deterministic :meth:`predict` calls.

        Args:
            input_dropout: Temporary feature-wise ODST-input dropout rate in
        ...
        """
        with self._temporary_dropout_rates(input_dropout=input_dropout, tree_dropout=tree_dropout):
            return self._predict_uncertainty(
                X,
        ...
        **kwargs,
        ...
        Args:
            ...
            uncertainty_for_opt: If True, return only a single uncertainty column
                ...
                epistemic ``knowledge_uncertainty`` when available, falling back to
                ``total_uncertainty`` for a flow head without dropout, which exposes
            ...
            knowledge_method: Epistemic uncertainty method for flow heads with
                dropout. ``"bald"`` (default) uses the entropy decomposition
                ``total - data``. ``"balsa_emd"`` uses a sampled BALSA-EMD style
                disagreement score computed from consecutive MC-dropout flow
                samples. Ignored for non-flow heads and overridden to
                ``"balsa_emd"`` when ``uncertainty_for_opt=True``.
        ...
        Returns:
            Union[pd.DataFrame, tuple[pd.DataFrame, np.ndarray]]:
                - If ``return_quantiles=False`` (default): A DataFrame with columns:
                    ...
                    - ``'data_uncertainty'``: Aleatoric uncertainty (from flow if available,
                                        ...
                                        - ``'total_uncertainty'``: Combined / primary uncertainty measure.
                                            For ``knowledge_method='balsa_emd'``, this is ``NaN`` because
                                            BALSA-EMD knowledge scores are not additive with entropy-based
                                            data uncertainty.
                ...
                - If ``uncertainty_for_opt=True``: ``pd.Series`` of
                  ``knowledge_uncertainty`` (epistemic) — BALSA-EMD scores for a
                  flow head with dropout, MC-dropout std/IQR for a non-flow head —
                  or of ``total_uncertainty`` when no epistemic estimate is
                  available (flow head without dropout).
        ...
        # Flow head without dropout: use flow uncertainty only
            ...
            self._warn_zero_dropout_mc("predict_uncertainty (flow head)")
            X_prep = self._prepare_data_for_node(X)
            self.module_.eval()
        ...
        if uncertainty_for_opt:
            ...
            # signal (BALSA-EMD for flow+dropout); fall back to total_uncertainty for
    ...
    def predict_with_combined_uncertainty(
        ...
        Args:
            ...
            knowledge_method: Epistemic uncertainty method for flow+dropout.
                ``"bald"`` uses the entropy decomposition ``total - data``
                (default, additive in nats). ``"balsa_emd"`` uses sampled
                BALSA-EMD disagreement between consecutive MC passes.
        ...
        Returns:
            ...
            If return_all=True:
                ...
                For ``knowledge_method='balsa_emd'``, ``total_uncertainty`` is
                ``NaN`` because EMD scores are not in entropy units and therefore
                cannot be added to ``data_uncertainty``.
        """
        # Check if using flow head
        is_flow_head = (
            hasattr(self, "module_") and hasattr(self.module_, "head_type") and self.module_.head_type == "flow"
        ...
        has_dropout = self._has_active_dropout(include_mlp=False)
        ...
        if not has_dropout:
            self._warn_zero_dropout_mc("predict_with_combined_uncertainty")
            self.module_.eval()
        ...
        if knowledge_method == "bald":
            ...
            # Sample-only BALSA-EMD style disagreement score from consecutive
            # MC-dropout flow passes. This is a ranking signal, not additive with
            # entropy terms, so total_uncertainty is marked unavailable (NaN).
            knowledge_uncertainty = balsa_emd_from_mc_samples(samples_by_batch, reduction="sum")
            total_uncertainty = np.full_like(data_uncertainty, np.nan, dtype=float)
    ...
    def default_parameters(self, prefix: str = "") -> Dict[str, Any]:
        ...
        if self.tune_head:
            ...
            defaults[prefix + "flow_type"] = "NSF"
            defaults[prefix + "flow_bins"] = 8
        elif self.head_type == "flow":
...
class NODEClassifier(BaseNODEEstimator, NeuralNetClassifier):
    ...
    def predict_uncertainty(
        ...
        **kwargs: Any,
        ...
        ``None`` preserves the rate fitted during training. A supplied rate in
        ``[0.0, 1.0)`` is used only for this MC-dropout call, then the fitted
        input/tree dropout rates are restored. The override does not alter
        MLP-head dropout or the deterministic :meth:`predict` contract.

        Args:
            input_dropout: Temporary feature-wise ODST-input dropout rate, or
                ``None`` to use the fitted rate.
            tree_dropout: Temporary whole-tree dropout rate, or ``None`` to use
                the fitted rate.
